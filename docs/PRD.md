...
### Goals
- Reconstruct a factual incident timeline from sampled logs with >85% accuracy.
- Generate at least 2 evidence-backed RCA hypotheses within 60 seconds of a trigger.
- Maintain zero AI cost by using free-tier API limits (Gemini 2.0 Flash / Groq).
- Enable "one-click" drafting for Jira and Slack.
- Persistent local memory for cross-incident analysis.
...
## 7. Core Product Flow
`Trigger (Webhook/Manual)` → `Sample Logs (100 lines)` → `Reconstruct Timeline (Gemini)` → `RCA Hypotheses (Gemini)` → `Impact Assessment (Gemini)` → `Memory Retrieval (FAISS)` → `Action Generation (Groq)` → `Final Operational Dashboard`.
...
- The system SHALL support manual log file uploads for offline analysis.
...
| :--- | :--- | :--- |
| **Reasoning** | Gemini 2.0 Flash | Best free-tier context window (1M+) and reasoning. |
| **Formatting** | Groq (Llama 3.1) | Sub-second latency for message drafting. |
| **Embeddings** | Gemini Text Embeddings | High quality, consistent with reasoning LLM. |
| **Vector Store** | FAISS | Local, lightweight, no-cost. |

---

## 11. Architecture Overview
RootSight V3 uses an async FastAPI backend serving a Next.js 15/Tailwind 4 frontend. The pipeline is orchestrated as a sequence of discrete modules that communicate via a shared SQLite database and persistent state.
...
| **RCA Accuracy** | > 80% | Must be reliable enough to trust evidence. |
