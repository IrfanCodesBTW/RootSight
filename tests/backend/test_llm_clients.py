...
from src.features.llm_clients.errors import LLMClientError
...
async def test_gemini_client_failure_after_retries():
    with patch("src.features.llm_clients.gemini_client.model.generate_content", side_effect=RuntimeError("offline")) as mock_generate:
        with patch("asyncio.sleep", new_callable=AsyncMock):
...
async def test_groq_client_failure_returns_none():
    with patch("src.features.llm_clients.groq_client.client.chat.completions.create", side_effect=RuntimeError("offline")) as mock_create:
        with patch("asyncio.sleep", new_callable=AsyncMock):
            result = await format_json("Test prompt")
