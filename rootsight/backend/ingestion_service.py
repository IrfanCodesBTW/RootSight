...
def _load_bundle_logs(file_to_load: str) -> list[dict[str, Any]]:
    bundle_path = Path(__file__).resolve().parent / "data" / "sample_incidents" / Path(file_to_load).name
...
async def ingest_logs(incident: Incident, bundle_file: str = None) -> List[RawEvent]:
    """
